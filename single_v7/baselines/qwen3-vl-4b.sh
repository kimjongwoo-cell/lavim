...
#######################################
# Qwen3-VL-4B-Thinking (thinking disabled) WSI-VQA inference + evaluation.
#
# Thinking control uses the backbone approach (wsi_latentmas/backbone/qwen3vl.py):
#   after apply_chat_template, "</think>" is appended to close the thinking
#   block before generation begins. This is unconditional and does not rely on
#   processor API kwargs.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 bash baselines/qwen3-vl-4b.sh
#
# Override checkpoint or input:
#   CHECKPOINT=/media/super/4TB/hj/Qwen3-VL-4B-Thinking \
#   TEST_JSON=/path/to/WsiVQA_test.json \
#   bash baselines/qwen3-vl-4b.sh
#######################################

REPO_ROOT="${REPO_ROOT:-/home/super/hj}"
HJ_ROOT="${HJ_ROOT:-$REPO_ROOT/HJ}"
LATENTMAS_ROOT="${LATENTMAS_ROOT:-$REPO_ROOT/wsi_latentmas_0709}"
GENERAL_EVALUATOR="${GENERAL_EVALUATOR:-$LATENTMAS_ROOT/eval/metrics.py}"
QWEN_ADAPTER="${QWEN_ADAPTER:-$LATENTMAS_ROOT/baselines/inference/qwen3-vl-4b-inference.py}"

TEST_JSON="${TEST_JSON:-/home/super/hj/HJ/WSI-VQA/dataset/WSI_captions/WsiVQA_test.json}"
INPUT_JSON="${INPUT_JSON:-$TEST_JSON}"
CHECKPOINT="${CHECKPOINT:-/media/super/4TB/hj/Qwen3-VL-4B-Thinking}"
...
DEFAULT_WORK_DIR="$RESULTS_ROOT/qwen3-vl-4b/${RUN_ID}"
WORK_DIR="${WORK_DIR:-$DEFAULT_WORK_DIR}"

THUMBNAIL_DIR="${THUMBNAIL_DIR:-$SHARED_THUMBNAIL_DIR}"
ANSWERS_FILE="${ANSWERS_FILE:-$WORK_DIR/predictions/qwen3vl_predictions.jsonl}"
VIS_DIR="${VIS_DIR:-$WORK_DIR/vis}"
EVAL_DIR="${EVAL_DIR:-$WORK_DIR/eval}"
EVAL_JSON="${EVAL_JSON:-$EVAL_DIR/qwen3vl_summary.json}"
EVAL_CSV="${EVAL_CSV:-$EVAL_DIR/qwen3vl_details.csv}"
GENERAL_OUTPUT_JSONL="${GENERAL_OUTPUT_JSONL:-$EVAL_DIR/qwen3vl_general_eval.jsonl}"
INFERENCE_TIME_JSON="${INFERENCE_TIME_JSON:-$EVAL_DIR/inference_time.json}"
GENERAL_EVAL_DIR="${GENERAL_EVAL_DIR:-$EVAL_DIR/general}"
GENERAL_EVAL_NAME="${GENERAL_EVAL_NAME:-qwen3_vl_4b_${DATASET_TAG}}"
GENERAL_EVAL_TXT="${GENERAL_EVAL_TXT:-$EVAL_DIR/general_eval.txt}"
LOG_DIR="${LOG_DIR:-$WORK_DIR/logs}"
RUN_LOG="${RUN_LOG:-$LOG_DIR/qwen3_vl_4b.log}"
...
SEED="${SEED:-0}"
SINGLE_NATIVE_THINKING="${SINGLE_NATIVE_THINKING:-false}"
...
check_inputs() {
  ...
  require_file "$QWEN_ADAPTER" "Qwen3-VL inference adapter"
  require_file "$GENERAL_EVALUATOR" "general WSI-VQA evaluator"

  if [[ -z "$LLM_URL" ]]; then
    require_dir "$CHECKPOINT" "Qwen3-VL checkpoint directory"
    require_file "$CHECKPOINT/config.json" "Qwen3-VL config"
  fi
  ...
  if [[ -z "$LLM_URL" ]]; then
    ...
    if ! python -c "import qwen_vl_utils" >/dev/null 2>&1; then
      ...
      log "Recommended for Qwen3-VL checkpoints: pip install qwen-vl-utils"
    fi
  fi
}
...
main() {
  setup_logging
  log "Qwen3-VL-4B (no thinking) pseudo baseline"
  log "Checkpoint: $CHECKPOINT"
  log "LLM URL: ${LLM_URL:-<local checkpoint mode>}"
  log "LLM model: ${LLM_MODEL:-<none>}"
  if flag_enabled "$SINGLE_NATIVE_THINKING"; then
    log "Thinking: enabled (native Qwen thinking)"
  else
    log "Thinking: disabled (backbone approach: </think> appended after apply_chat_template)"
  fi

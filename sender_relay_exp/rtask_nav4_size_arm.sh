#!/bin/bash
# Copy of rtask_nav4_arm.sh with the model selectable: env NAV4_MODEL=<model dir> (default 4B).
# Use a size-specific OUTROOT (runs/nav4_2b, runs/nav4_8b) so sizes never share a gate or results.
...
MODEL=${NAV4_MODEL:-/home/users/whddn12316/models/Qwen3-VL-4B-Thinking}
...
navrep() {  # cases whose Navigator needed a format repair (0 = every selection parsed first time)
  $PY - "$1" <<'PYEOF'
...
log "CLAIM gpu$G shard=$SHARD arm=$ARM variant=$VARIANT model=$(basename $MODEL) planner_task=${VLMAS_PLANNER_TASK:-default} nav4_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_nav4.py | cut -c1-8) navigator_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) eovc_md5=$(md5sum $REPO/memory/eovc.py | cut -c1-8)"
...
run() {
  ...
  local rc=$?
  log "DONE $ARM $tag rc=$rc results=$(find $out -name result.json | wc -l) nav4=$(grep -l 'independent of the x5 selection' $(find $out -name navigator_calls.json) 2>/dev/null | wc -l) clamp=$(grep -c 'NAV3-X5CLAMP' $out.log) eovc=$(grep -c '^\[EOVC\]' $out.log) prunev4=$(grep -c 'PruneVision/v4' $out.log) nav_repaired=$(navrep $out) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  ev=$(grep -c '^\[EOVC\]' $o.log); v4=$(grep -c 'PruneVision/v4' $o.log)
  need_ev=0; [ "$ARM" = eovc ] && need_ev=$v4
  need_v4=0; { [ "$ARM" = eovc ] || [ "$ARM" = pruneb ]; } && need_v4=1
  okv4=1; [ "$need_v4" -eq 1 ] && { [ "$v4" -gt 0 ] || okv4=0; }
  nr=$(navrep $o)
  if [ "$r" -eq 2 ] && [ "$n4" -gt 0 ] && [ "$nr" -eq 0 ] && [ "$t" -eq 0 ] && [ "$em" -eq 0 ] \
     && [ "$ev" -eq "$need_ev" ] && [ "$okv4" -eq 1 ]; then
    echo "PASS results=$r nav4=$n4 nav_repaired=$nr prunev4=$v4 eovc=$ev empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  else
    echo "FAIL results=$r nav4=$n4 nav_repaired=$nr prunev4=$v4 eovc=$ev need_eovc=$need_ev need_prunev4=$need_v4 empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  fi

#!/bin/bash
# 0916 VISION-ZERO (C1 예산 1토큰): 디코딩 수정 기준선에서 Reasoner 프리필 visual 토큰을 3072→1로 줄여 같은 문항의 답이 바뀌는지 본다. 원래는 CEILING: 디코딩 수정 기준선을 seed별로 샘플링해 "한 번이라도 맞히는" 상한을 재는 런. 0913-board Latent base (nav3, patch 12, step 10, NO C1/NTRS), greedy decode (same as board base)
...
set -u
SEED=${1:?seed}; SHARD=${2:?shard}; NSHARD=${3:?nshard}; GPUS=${4:?gpus}; shift 4; SPECS=${*:-gtex:190}; HGPU=; HPID=
...
K=$REPO/sender_relay_exp/runs/rtask_novis
...
run() {
  ...
  cd $REPO && env \
    CUDA_VISIBLE_DEVICES=$G PYTHONPATH=$REPO/tmp_hj/lpvh_ntrs_hooks:$REPO PYTHONUNBUFFERED=1 \
    VLMAS_PROMPT_SET=prompt1 VLMAS_ATTN_IMPLEMENTATION=sdpa VLMAS_NAV2=1 VLMAS_NAV3_INDEPENDENT=1 \
    VLMAS_CROSS_SCALE_ROUTER=0 VLMAS_EFF_DUMP=1 VLMAS_REASONER_TASK="$RTASK" VLMAS_ANSWERER_GRADE_RULE=1 VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.0001 VLMAS_C1_META_BRIDGE=1 \
...
for spec in $SPECS; do
  ...
  for ((i=0; i<n; i+=STEP)); do
    [ $(( (i / STEP) % NSHARD )) -eq "$SHARD" ] || continue
...
log "ALL DONE seed $SEED shard $SHARD"

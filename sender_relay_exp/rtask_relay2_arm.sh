...
if [ "$SHARD" = smoke ]; then
  ...
  t=$(grep -c Traceback $o.log); c=$(grep -cE '^\[(LRPSR|PSFP|LADR)\] case' $o.log); rel2=$(grep -c 'LatentKVRelay2' $o.log); rl=$(grep -c '^\[reallocate\]' $o.log); sk=$(grep -cE '^\[(LRPSR|PSFP|LADR)\] SKIP' $o.log); em=$(grep -c 'EMPTY terminal answer' $o.log)

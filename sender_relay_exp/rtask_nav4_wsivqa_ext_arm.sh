...
if [ "$SHARD" = smoke ]; then
  if [ "$ARM" = plipext ]; then   # the ranking key uses the real WSI-VQA index, so smoke on wsivqa.json #0 and #7 (same 2 questions)
  ...
  pt=$(grep -c '^\[PlanTargets\]' $o.log); need_pt_zero=0; { [ "$ARM" = nonav2 ] || [ "$ARM" = latplan ] || [ "$ARM" = plipext ]; } && need_pt_zero=1

#!/bin/bash
# nav3 + C1 #14.6 NTRS 50% (VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5) + Dual Relay2
#   (--variant latent_kv_relay2_dual: visual-grounded latent KV relay at navigator and reasoner, the relay half of the
#   0912 dashboard "Nav3 Both"; Pruning-B is replaced by NTRS). VLMAS_EFF_DUMP=1. Default dataset: GTEx (190).
#   Compare with nav3 base, NTRS 50% alone (runs/ntrs50_nav3) and Nav3 Both (runs/nav3_prompt1_both_*).
# Smoke gate (once, shared lock): gtex2 cases 0-1 -> results 2, [NTRS] kept 1536/3072 x2, [LatentKVRelay2] >= 2, tb 0, eff 2.
# usage: relay2_ntrs50_eff.sh <shard> <nshard> "<gpu list>" [dataset:n ...]
set -u
...
K=$REPO/sender_relay_exp/runs/relay2_ntrs50_nav3
...
log "CLAIM gpu$G shard=$SHARD keep=0.5 variant=latent_kv_relay2_dual presets_md5=$(md5sum $REPO/vision_text_mas/direct_hf_ablation_presets.py | cut -c1-8) roots_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_roots.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) ssda_md5=$(md5sum $REPO/memory/ssda_select.py | cut -c1-8) engine_md5=$(md5sum $REPO/vision_text_mas/latent_qwen_engine.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) nav3_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8)"

run() {  # run <tag> <dataset-json> <dest> <ids...>
  local tag=$1 dsjson=$2 dest=$3; shift 3
  local out=$dest/gpu${G}_attempt_$(date +%Y%m%d_%H%M%S)
  mkdir -p $dest
  log "START $tag keep=0.5 relay2_dual pending=$(( $# / 2 )) -> ${out#$K/}"
  cd $REPO && env \
    ...
    --variant latent_kv_relay2_dual --backbone qwen3-vl --dataset "$dsjson" --slide-root "$DATA/slides" \
  ...
  local rc=$?
  LAST_OUT=$out
  log "DONE $tag rc=$rc results=$(find $out -name result.json | wc -l) ntrs=$(grep -c '^\[NTRS\] kept' $out.log) kept50=$(grep -c '^\[NTRS\] kept 1536/3072' $out.log) relay2=$(grep -c '^\[LatentKVRelay2\]' $out.log) relay2_nav=$(grep -c '^\[LatentKVRelay2:navigator\]' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ $SMOKER = 1 ]; then
  ...
  rl=$(grep -c '^\[LatentKVRelay2\]' $o.log)
  line="results=$r kept1536=$k relay2=$rl tb=$t eff=$e $(grep -m1 '^\[LatentKVRelay2\]' $o.log | cut -c1-140)"
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$rl" -ge 2 ] && [ "$t" -eq 0 ] && [ "$e" -eq 2 ]; then echo "PASS $line" > $GATE; else echo "FAIL $line" > $GATE; fi
  log "SMOKE $(cat $GATE)"

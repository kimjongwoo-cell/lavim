#!/bin/bash
#   ARM=ntrs75 : NTRS 75% (VLMAS_C1_QASC=1 SELECT=ntrs KEEP=0.75) -> runs/rtask_ntrs75_nav3; starts after the NTRS 50% move to 6/7/8
#   Everything else = greedy_base_rtask_full.sh (Latent base + 디코딩 수정).
set -u
ARM=${1:?arm}; SHARD=${2:?shard}; NSHARD=${3:?nshard}; GPUS=${4:?gpus}; shift 4; SPECS=${*:-tcga_expert_vqa:128 tcga_slidebench:197 gtex:190 tcga:221 panda:196}; HGPU=; HPID=
...
case $ARM in
  ntrs75) K=$REPO/sender_relay_exp/runs/rtask_ntrs75_nav3; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.75 VLMAS_C1_META_BRIDGE=1" ;;
  *) echo bad arm; exit 2 ;;
esac
...
M50=$REPO/sender_relay_exp/runs/rtask_ntrs50_nav3/move678.log
log "WAIT NTRS50 moved to 6/7/8"
while ! grep -q "relaunched shards 0 2" $M50 2>/dev/null; do sleep 30; done
sleep 120
if [ "$SHARD" != smoke ]; then
  log "WAIT smoke gate"
...
run() {
  ...
  local rc=$?
  log "DONE $tag rc=$rc results=$(find $out -name result.json | wc -l) ntrs2304=$(grep -c '^\[NTRS\] kept 2304' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  r=$(find $o -name result.json | wc -l); k=$(grep -c '^\[NTRS\] kept 2304/3072' $o.log); t=$(grep -c Traceback $o.log)
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$t" -eq 0 ]; then echo "PASS results=$r kept2304=$k tb=$t" > $K/SMOKE_GATE; else echo "FAIL results=$r kept2304=$k tb=$t" > $K/SMOKE_GATE; fi

...
#   ROOT runs/nav4_rho_latplan, own claim dir, hooks tmp_hj/nova_rho_latplan_hooks (chains lpvh_ntrs_hooks, then patches secld_select.evidence).
...
HOOKS=$REPO/tmp_hj/nova_rho_latplan_hooks
ROOT=${OUTROOT:-$REPO/sender_relay_exp/runs/nav4_rho_latplan}
VARIANT=base
case $ARM in
  ...
  nova_rho_latplan) K=$ROOT/nova_rho_latplan; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_NOVA_KEEP=0.25 VLMAS_C1_META_BRIDGE=1 VLMAS_NOVA_RHO=1 VLMAS_PLAN_LATENT_ONLY=1" ;;
...
log "CLAIM gpu$G shard=$SHARD arm=$ARM variant=$VARIANT nav4_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_nav4.py | cut -c1-8) navigator_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) eovc_md5=$(md5sum $REPO/memory/eovc.py | cut -c1-8) secld_md5=$(md5sum $REPO/memory/secld_select.py | cut -c1-8) nova_md5=$(md5sum $REPO/memory/nova_select.py | cut -c1-8) nova_rho_md5=$(md5sum $REPO/memory/nova_rho_select.py | cut -c1-8) hook_md5=$(md5sum $HOOKS/sitecustomize.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) used=$(mem $G)"
...
run() {
  ...
  local rc=$?
  log "DONE $ARM $tag rc=$rc results=$(find $out -name result.json | wc -l) nav4=$(grep -l 'independent of the x5 selection' $(find $out -name navigator_calls.json) 2>/dev/null | wc -l) clamp=$(grep -c 'NAV3-X5CLAMP' $out.log) eovc=$(grep -c '^\[EOVC\]' $out.log) secld=$(grep -c '^\[SECLD\]' $out.log) nova=$(grep -c '^\[NOVA\] kept' $out.log) nova3=$(grep -c '^\[NOVARHO\] n=' $out.log) nova3_patched=$(grep -c '^\[NOVARHO\] patched' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) empty_crops=$(grep -o 'empty_crops=[0-9]*' $out.log | grep -vc 'empty_crops=0') prunev4=$(grep -c 'PruneVision/v4' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  ev=$(grep -c '^\[EOVC\]' $o.log); v4=$(grep -c 'PruneVision/v4' $o.log); sl=$(grep -c '^\[SECLD\]' $o.log); nv=$(grep -c '^\[NOVA\] kept' $o.log); n2=$(grep -c '^\[NOVARHO\] n=' $o.log); n2p=$(grep -c '^\[NOVARHO\] patched' $o.log)
  need_nv=0; { [ "$ARM" = nova ] || [ "$ARM" = nova_rho_latplan ]; } && need_nv=2
  need_n2=0; [ "$ARM" = nova_rho_latplan ] && need_n2=2

...
if [ "$SHARD" = smoke ]; then
  ...
  need_nv=0; { [ "$ARM" = nova ] || [ "$ARM" = nova_fulr ] || [ "$ARM" = glvr_rho ] || [ "$ARM" = glvr_rho_nf ] || [ "$ARM" = glvr_rho_fast ]; } && need_nv=$SMOKE_N
  ...
  if [ "$r" -eq "$SMOKE_N" ] && [ "$n4" -gt 0 ] && [ "$t" -eq 0 ] && [ "$em" -eq 0 ] \
    ...
    echo "FAIL plantargets=$pt results=$r nav4=$n4 prunev4=$v4 eovc=$ev secld=$sl nova=$nv fulr=$fu fulr_skip=$fs need_nova=$need_nv need_fulr=$need_fu need_eovc=$need_ev need_secld=$need_sl need_prunev4=$need_v4 empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  fi

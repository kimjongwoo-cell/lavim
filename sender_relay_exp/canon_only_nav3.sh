#!/bin/bash
# Control for canon_reread_nav3.sh: nav3 · Canonical addressing ONLY (no Context-Updated Re-Read).
# Identical to canon_reread_nav3.sh except VLMAS_ANSWERER_PFR* removed, output runs/canon_only_nav3/, and dataset
# order ExpertVQA -> SlideBench -> GTEx -> TCGA -> PANDA (the two sets that dropped first). Separates the share of
# the canon_reread drop that comes from canonical addressing from the share that comes from the re-read.
...
K=$REPO/sender_relay_exp/runs/canon_only_nav3
...
run() {
  ...
  local pfr; pfr=$(grep -c '^\[PFRctx\]\|^\[PFR:' $out.log)
...
if [ "$ROLE" = smoke ]; then
  ...
  if [ "$res" -eq 2 ] && [ "$canon" -eq 2 ] && [ "$pfr" -eq 0 ] && [ "$tb" -eq 0 ]; then

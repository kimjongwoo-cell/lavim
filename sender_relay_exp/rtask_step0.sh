#!/bin/bash
# 0916 latent step 0 (Reasoner VLMAS_REASONER_LATENT_STEPS=0) on the decode-fix default: 0913-board Latent base (nav3, patch 12, step 10, NO C1/NTRS), greedy decode (same as board base)
...
K=$REPO/sender_relay_exp/runs/rtask_step0_nav3
...
W75=$REPO/sender_relay_exp/runs/rtask_ntrs75_nav3
log "WAIT ntrs75 all shards done"
while ! { [ -f $W75/SHARD0_DONE ] && [ -f $W75/SHARD1_DONE ] && [ -f $W75/SHARD2_DONE ]; }; do sleep 60; done
if [ "$SHARD" != smoke ]; then
  log "WAIT smoke gate"
...
run() {
  ...
  cd $REPO && env \
    ...
    VLMAS_CROSS_SCALE_ROUTER=0 VLMAS_EFF_DUMP=1 VLMAS_REASONER_TASK="$RTASK" VLMAS_REASONER_LATENT_STEPS=0 \
...
if [ "$SHARD" = smoke ]; then
  ...
  r=$(find $o -name result.json | wc -l); z=$(grep -c "latent steps 0\|\[Reasoner\]" $o.log); t=$(grep -c Traceback $o.log)
  if [ "$r" -eq 2 ] && [ "$t" -eq 0 ]; then echo "PASS results=$r tb=$t" > $K/SMOKE_GATE; else echo "FAIL results=$r tb=$t" > $K/SMOKE_GATE; fi

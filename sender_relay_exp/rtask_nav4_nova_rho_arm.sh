...
#   ROOT runs/nav4_rho, own claim dir, hooks tmp_hj/nova_rho_hooks (chains lpvh_ntrs_hooks, then patches secld_select.evidence).
...
HOOKS=$REPO/tmp_hj/nova_rho_hooks
ROOT=${OUTROOT:-$REPO/sender_relay_exp/runs/nav4_rho}
VARIANT=base
case $ARM in
  ...
  nova_rho) K=$ROOT/nova_rho; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_NOVA_KEEP=0.25 VLMAS_C1_META_BRIDGE=1 VLMAS_NOVA_RHO=1" ;;
...
if [ "$SHARD" = smoke ]; then
  ...
  need_nv=0; { [ "$ARM" = nova ] || [ "$ARM" = nova_rho ]; } && need_nv=2
  need_n2=0; [ "$ARM" = nova_rho ] && need_n2=2

...
if [ "$SHARD" = smoke ]; then
  ...
  t=$(grep -c Traceback $o.log); c=$(grep -cE '^\[(LRPSR|PSFP|LADR)\] case' $o.log); sk=$(grep -cE '^\[(LRPSR|PSFP|LADR)\] SKIP' $o.log); em=$(grep -c 'EMPTY terminal answer' $o.log)
  md=$(grep -oE '(maxdiff_identity=[0-9.e+-]*|activated=[0-9]+ \([0-9.]+%\) argmax_flips=[0-9]+)' $o.log | tail -2 | tr '\n' ' ')
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$t" -eq 0 ] && [ "$c" -eq 2 ] && [ "$sk" -eq 0 ] && [ "$em" -eq 0 ]; then
    echo "PASS results=$r kept1536=$k case=$c skip=$sk empty=$em tb=$t $md" > $K/SMOKE_GATE_$ARM
  else
    echo "FAIL results=$r kept1536=$k case=$c skip=$sk empty=$em tb=$t $md" > $K/SMOKE_GATE_$ARM
  fi

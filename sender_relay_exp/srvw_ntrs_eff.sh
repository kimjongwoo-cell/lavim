#!/bin/bash
# nav3 + C1 #14.6 NTRS + C2 11.9 Support-Resolved Visual Write Handoff (memory/srvw.py, VLMAS_SRVW=1, boundary=last,
#   layers=all), VLMAS_EFF_DUMP=1, PANDA with the nav3 x5 clamp. Base for Δ = runs/ntrs_nav3 (same questions).
# Import hooks tmp_hj/lpvh_ntrs_hooks/sitecustomize.py: VLMAS_C1_META_BRIDGE=1 (C1 keep mask -> _visual_meta, without it
#   SRVW SKIPs every NTRS case like LPVH), VLMAS_NAV3_X5_CLAMP=1 for PANDA.
# Smoke gate (once, shared lock): gtex2 cases 0-1, identity (maxdiff < 0.05) then SRVW=1; requires [C1META] used=True
#   meta=ok, [SRVW] case lines, no [SRVW] SKIP, tb 0.
# usage: srvw_ntrs_eff.sh smoke "<gpu list>"          smoke only
#        srvw_ntrs_eff.sh <shard> <nshard> "<gpu list>"  smoke gate then 5 full sets (resumable)
set -u
if [ "${1:?shard|smoke}" = smoke ]; then SMOKE_ONLY=1; SHARD=smoke; NSHARD=1; GPUS=${2:?gpus}; else SMOKE_ONLY=0; SHARD=$1; NSHARD=${2:?nshard}; GPUS=${3:?gpus}; fi
...
K=$REPO/sender_relay_exp/runs/srvw_ntrs_nav3
...
[ $SMOKE_ONLY = 1 ] && [ $SMOKER = 0 ] && { log "smoke-only: gate already exists ($(cat $GATE))"; exit 0; }
...
log "CLAIM gpu$G shard=$SHARD srvw_md5=$(md5sum $REPO/memory/srvw.py | cut -c1-8) engine_md5=$(md5sum $REPO/vision_text_mas/latent_qwen_engine.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) ssda_md5=$(md5sum $REPO/memory/ssda_select.py | cut -c1-8) bridge_md5=$(md5sum $HOOKS/c1_meta_bridge.py | cut -c1-8) nav3_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) roots_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_roots.py | cut -c1-8)"

# run <tag> <srvw 1|identity> <clamp 0|1> <dataset-json> <dest> <ids...>
run() {
  local tag=$1 lmode=$2 clamp=$3 dsjson=$4 dest=$5; shift 5
  local out=$dest/gpu${G}_attempt_$(date +%Y%m%d_%H%M%S)
  mkdir -p $dest
  log "START $tag srvw=$lmode clamp=$clamp pending=$(( $# / 2 )) -> ${out#$K/}"
  cd $REPO && env \
    ...
    VLMAS_SRVW=$lmode VLMAS_SRVW_BOUNDARY=last VLMAS_SRVW_LAYERS=all \
  ...
  local rc=$?
  LAST_OUT=$out
  log "DONE $tag srvw=$lmode clamp=$clamp rc=$rc results=$(find $out -name result.json | wc -l) ntrs=$(grep -c '^\[NTRS\] kept' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) srvw_case=$(grep -c '^\[SRVW\] case' $out.log) srvw_skip=$(grep -c '^\[SRVW\] SKIP' $out.log) clamp_active=$(grep -c '^\[NAV3-X5CLAMP\] active' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ $SMOKER = 1 ]; then
  ...
  i_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $o); i_case=$(grep -c '^\[SRVW\] case' $o); i_skip=$(grep -c '^\[SRVW\] SKIP' $o); i_tb=$(grep -c Traceback $o)
  run smoke_srvw 1 0 $SM $K/smoke/srvw --dataset-index 0 --dataset-index 1
  o=$LAST_OUT.log
  r=$(find $LAST_OUT -name result.json | wc -l); ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $o); lc=$(grep -c '^\[SRVW\] case' $o)
  sk=$(grep -c '^\[SRVW\] SKIP' $o); tb=$(grep -c Traceback $o); ef=$(find $LAST_OUT -name efficiency.json | wc -l)
  line="identity_text_same_as_native=$SAME identity_maxdiff=$MD identity(c1meta=$i_ok case=$i_case skip=$i_skip tb=$i_tb) srvw(results=$r c1meta=$ok case=$lc skip=$sk tb=$tb eff=$ef)"
  ...
  grep -h '^\[SRVW\]\|^\[C1META\]' $K/smoke/*/gpu*_attempt_*.log | cut -c1-600 >> $LOG
  log "SMOKE $(cat $GATE)"
  grep -q ^PASS $GATE || exit 1
fi
[ $SMOKE_ONLY = 1 ] && { log "SMOKE ONLY done"; exit 0; }

#!/bin/bash
# 09-19 LATMASK copy of rtask_nav4_wsivqa_vn6g2_arm.sh: arm latmask_l0 = latplan base + Answerer layer-0 attention to the
#   Planner/Navigator latent columns masked (tmp_hj/latmask_hooks). broken18/clean18 gate: correct 4->8 / 12->10,
#   garbled 12->2. ROOT runs/nav4_wsivqa_nc/latmask_l0, claims runs/nav4_wsivqa_nc/claims_latmask, all 735.
# 0919 VN6G2 copy of rtask_nav4_wsivqa_nc_arm.sh: adds arm nova_vn6g2_latplan = C1 NOVA with rho -> 1 - value norm (vision L6, per-question min-max),
#   Gaussian sigma 2, keep 0.25, latplan (env/hooks identical to rtask_nav4_c1int_latplan_arm.sh arm nova_vn6g2_latplan, hooks tmp_hj/nova_lgv_latplan_hooks).
# 0919 08:0x + arm nova_vn6g2_merge_latplan = same + VLMAS_NOVA_MERGE=1 (dropped tokens folded into nearest kept token of the same crop,
#   norm-preserving; hook code added by the other session 07:41). The vn6g2-only run was stopped at 17 results (user: run 1번+merge).
#   All 735 questions (own block plan in $K), own claim dir claims_vn6g2, --no-canonical-open-options (WSI-VQA default). Else identical.
# 0917 WSI-VQA copy of rtask_nav4_nonav2_arm.sh (arms base / latplan used); only DATA, SPECS, ROOT and the smoke json differ.
# 0919 NC copy of rtask_nav4_wsivqa_sel_arm.sh: passes --no-canonical-open-options (the CLI default is ON, so merely omitting the flag
#   changes nothing — the 05:28 run did that and was moved to runs/nav4_wsivqa_nc/_invalid_default_canonical_on), ROOT runs/nav4_wsivqa_nc,
#   smoke gate needs >=1 Choice list (smoke #7 is open now); else identical. Only the 269 label-open questions are re-run.
...
{ [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ]; } && HOOKS=$REPO/tmp_hj/nova_lgv_latplan_hooks
[ "$ARM" = latmask_l0 ] && HOOKS=$REPO/tmp_hj/latmask_hooks
...
ROOT=${OUTROOT:-$REPO/sender_relay_exp/runs/nav4_wsivqa_nc}
VARIANT=base
case $ARM in
  ...
  nova_vn6g2_latplan) K=$ROOT/nova_vn6g2_latplan; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_NOVA_KEEP=0.25 VLMAS_C1_META_BRIDGE=1 VLMAS_NOVA_LGV=1 VLMAS_NOVA_SIGNAL=vn6g2 VLMAS_PLAN_LATENT_ONLY=1" ;;
  nova_vn6g2_merge_latplan) K=$ROOT/nova_vn6g2_merge_latplan; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_NOVA_KEEP=0.25 VLMAS_C1_META_BRIDGE=1 VLMAS_NOVA_LGV=1 VLMAS_NOVA_SIGNAL=vn6g2 VLMAS_NOVA_MERGE=1 VLMAS_PLAN_LATENT_ONLY=1" ;;
  latmask_l0) K=$ROOT/latmask_l0; ARMENV="VLMAS_PLAN_LATENT_ONLY=1 VLMAS_LATMASK=l0 VLMAS_LATMASK_ROLES=P,N" ;;
...
{ [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ]; } && CLAIM=${CLAIMDIR:-$ROOT/claims_vn6g2}
[ "$ARM" = latmask_l0 ] && CLAIM=${CLAIMDIR:-$ROOT/claims_latmask}
...
# (nc copy: no wait-for-plipext gate; arms are chained one at a time by tmp_hj/nc_chain.sh)
...
log "CLAIM gpu$G shard=$SHARD arm=$ARM variant=$VARIANT nav4_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_nav4.py | cut -c1-8) navigator_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) eovc_md5=$(md5sum $REPO/memory/eovc.py | cut -c1-8) secld_md5=$(md5sum $REPO/memory/secld_select.py | cut -c1-8) nova_md5=$(md5sum $REPO/memory/nova_select.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) rnlcr_md5=$(md5sum $REPO/memory/rnlcr.py | cut -c1-8) lu_md5=$(md5sum $REPO/memory/lu.py | cut -c1-8) engine_md5=$(md5sum $REPO/vision_text_mas/latent_qwen_engine.py | cut -c1-8) nova_lgv_md5=$(md5sum $HOOKS/nova_lgv.py 2>/dev/null | cut -c1-8) hook_md5=$(md5sum $HOOKS/sitecustomize.py | cut -c1-8) env=[$ARMENV] used=$(mem $G)"
...
run() {
  ...
  cd $REPO && env \
    ...
    --no-canonical-open-options $NAVKV --io-pipeline --no-save-navigation-pngs --case-retries 0 \
    "$@" > "$out.log" 2>&1 < /dev/null
  local rc=$?
  log "DONE $ARM $tag rc=$rc results=$(find $out -name result.json | wc -l) nav4=$(grep -l 'independent of the x5 selection' $(find $out -name navigator_calls.json) 2>/dev/null | wc -l) clamp=$(grep -c 'NAV3-X5CLAMP' $out.log) eovc=$(grep -c '^\[EOVC\]' $out.log) secld=$(grep -c '^\[SECLD\]' $out.log) nova=$(grep -c '^\[NOVA\] kept' $out.log) nova3=$(grep -c '^\[NOVALGV\] n=' $out.log) nova3_patched=$(grep -c '^\[NOVALGV\] patched' $out.log) latplan=$(grep -c '^\[LATPLAN\] patched' $out.log) merge=$(grep -c '^\[NOVAMERGE\]' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) empty_crops=$(grep -o 'empty_crops=[0-9]*' $out.log | grep -vc 'empty_crops=0') prunev4=$(grep -c 'PruneVision/v4' $out.log) fulr=$(grep -c '^\[RNLCR\] case .* mode=fulr applied=True' $out.log) fulr_skip=$(grep -c '^\[RNLCR.*SKIP' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  need_nv=0; { [ "$ARM" = nova ] || [ "$ARM" = nova_fulr ] || [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ]; } && need_nv=2
  n2=$(grep -c '^\[NOVALGV\] n=' $o.log); n2p=$(grep -c '^\[NOVALGV\] patched' $o.log); lp=$(grep -c '^\[LATPLAN\] patched' $o.log)
  lmk=$(grep -c "^\[LATMASK\] case" $o.log); [ "$ARM" = latmask_l0 ] && [ "$lmk" -ne 2 ] && n4=0
  mg=$(grep -c '^\[NOVAMERGE\]' $o.log); need_mg=0; [ "$ARM" = nova_vn6g2_merge_latplan ] && need_mg=2
  ok2=1; if [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ]; then { [ "$n2" -eq 2 ] && [ "$n2p" -gt 0 ] && [ "$lp" -eq 2 ] && [ "$mg" -eq "$need_mg" ]; } || ok2=0; else [ "$n2" -eq 0 ] || ok2=0; fi
  fu=$(grep -c '^\[RNLCR\] case .* mode=fulr applied=True' $o.log); fs=$(grep -c '^\[RNLCR.*SKIP' $o.log)
  pt=$(grep -c '^\[PlanTargets\]' $o.log); need_pt_zero=0; { [ "$ARM" = nonav2 ] || [ "$ARM" = latplan ] || [ "$ARM" = plipext ] || [ "$ARM" = plipnp ] || [ "$ARM" = navnp ] || [ "$ARM" = navsel ] || [ "$ARM" = plipsel ] || [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ] || [ "$ARM" = latmask_l0 ]; } && need_pt_zero=1
  ...
  if [ "$cl" -eq 1 ] && [ "$cn" -eq 0 ] && [ "$r" -eq 2 ] && [ "$n4" -gt 0 ] && [ "$t" -eq 0 ] && [ "$em" -eq 0 ] \
     && [ "$ev" -eq "$need_ev" ] && [ "$sl" -eq "$need_sl" ] && [ "$nv" -eq "$need_nv" ] && [ "$okv4" -eq 1 ] \
     && [ "$fu" -eq "$need_fu" ] && [ "$fs" -eq 0 ] \
     && [ "$ok2" -eq 1 ] && { [ "$need_pt_zero" -eq 0 ] || [ "$pt" -eq 0 ]; }; then
    echo "PASS choicelist=$cl CHOICEn=$cn results=$r nav4=$n4 prunev4=$v4 eovc=$ev secld=$sl nova=$nv fulr=$fu fulr_skip=$fs plantargets=$pt nova3=$n2 nova3_patched=$n2p latplan=$lp merge=$mg empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  else
    echo "FAIL choicelist=$cl CHOICEn=$cn plantargets=$pt results=$r nav4=$n4 prunev4=$v4 eovc=$ev secld=$sl nova=$nv fulr=$fu fulr_skip=$fs need_nova=$need_nv need_fulr=$need_fu need_eovc=$need_ev need_secld=$need_sl need_prunev4=$need_v4 nova3=$n2 nova3_patched=$n2p latplan=$lp merge=$mg empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  fi

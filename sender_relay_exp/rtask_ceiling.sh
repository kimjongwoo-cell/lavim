#!/bin/bash
# 0916 CEILING (sampling any-correct): 디코딩 수정 기준선을 seed별로 샘플링해 "한 번이라도 맞히는" 상한을 재는 런. 0913-board Latent base (nav3, patch 12, step 10, NO C1/NTRS), greedy decode (same as board base)
...
K=$REPO/sender_relay_exp/runs/rtask_ceiling/seed$SEED
...
run() {
  ...
  cd $REPO && env \
    ...
    --temperature 0.6 --top-p 0.95 --seed $SEED --no-answerer-thinking \

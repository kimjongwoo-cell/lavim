...
[ "$ARM" = plipnp ] && CLAIM=${CLAIMDIR:-$ROOT/claims_plipnp}   # own GPU claims: never blocks or is blocked by plipext claims
...
if [ "$SHARD" = smoke ]; then
  ...
  pt=$(grep -c '^\[PlanTargets\]' $o.log); need_pt_zero=0; { [ "$ARM" = nonav2 ] || [ "$ARM" = latplan ] || [ "$ARM" = plipext ] || [ "$ARM" = plipnp ]; } && need_pt_zero=1

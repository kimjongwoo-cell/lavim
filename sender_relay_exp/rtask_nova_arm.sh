#!/bin/bash
# C1 #21 NOVA arms on Latent base + 디코딩 수정 + nav3 (= rtask_ntrs50_arm.sh with the C1 selector swapped).
#   ARM=nova         : NOVA 25%  (VLMAS_C1_QASC=1 SELECT=nova KEEP=0.25)              -> runs/rtask_nova_nav3
#   ARM=nova50       : NOVA 50%                                                        -> runs/rtask_nova50_nav3
#   ARM=nova_logdet  : e_i == 1 pure support log-det, 25%  (page §8 falsifier 4)       -> runs/rtask_nova_logdet_nav3
#   ARM=nova_etopk   : e_i Top-k, 25%                      (page §8 falsifier 3)       -> runs/rtask_nova_etopk_nav3
# usage: [MAXUSED=<MiB>] rtask_nova_arm.sh <arm> <smoke|shard> <nshard> "<gpus>" [ds:n ...]   (default gtex:190)
# MAXUSED (default 2000) = a GPU counts as free below this used-MiB, so a shard can co-locate on a busy GPU.
# Smoke gate: results 2, [NOVA] kept lines 2, Traceback 0, EMPTY terminal answer 0.
set -u
...
COMMON="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_C1_META_BRIDGE=1"
case $ARM in
  nova)        K=$REPO/sender_relay_exp/runs/rtask_nova_nav3;        ARMENV="$COMMON VLMAS_NOVA_KEEP=0.25" ;;
  nova50)      K=$REPO/sender_relay_exp/runs/rtask_nova50_nav3;      ARMENV="$COMMON VLMAS_NOVA_KEEP=0.5" ;;
  nova_logdet) K=$REPO/sender_relay_exp/runs/rtask_nova_logdet_nav3; ARMENV="$COMMON VLMAS_NOVA_KEEP=0.25 VLMAS_NOVA_MODE=logdet" ;;
  nova_etopk)  K=$REPO/sender_relay_exp/runs/rtask_nova_etopk_nav3;  ARMENV="$COMMON VLMAS_NOVA_KEEP=0.25 VLMAS_NOVA_MODE=etopk" ;;
  *) echo bad arm; exit 2 ;;
esac
...
G=""
# HANDOVER: HPID=<launcher pid> HGPU=<gpu> -> wait for that launcher to exit, then take the claim it releases
# (its EXIT trap rmdir's it) before anyone polling at 60 s can; falls back to normal polling after 180 s.
HGPU=${HGPU:-}; HPID=${HPID:-}
if [ -n "$HGPU" ] && [ -n "$HPID" ]; then
  log "HANDOVER wait for pid $HPID on gpu$HGPU then take its claim"
  while kill -0 "$HPID" 2>/dev/null; do sleep 2; done
  for i in $(seq 1 180); do
    if mkdir $CLAIM/claim_gpu$HGPU 2>/dev/null; then G=$HGPU; break; fi
    sleep 1
  done
  log "HANDOVER result gpu=${G:-none} used=$(mem $HGPU)"
fi
if [ "$SHARD" != smoke ]; then
  log "WAIT smoke gate"
...
[ "$SHARD" = smoke ] && rm -f $K/SMOKE_GATE
trap 'rmdir $CLAIM/claim_gpu$G 2>/dev/null' EXIT
log "CLAIM gpu$G used=$(mem $G) shard=$SHARD arm=$ARM env=[$ARMENV] nav3_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) nova_md5=$(md5sum $REPO/memory/nova_select.py | cut -c1-8) secld_md5=$(md5sum $REPO/memory/secld_select.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) engine_md5=$(md5sum $REPO/vision_text_mas/latent_qwen_engine.py | cut -c1-8)"
...
run() {
  ...
  local rc=$?
  log "DONE $tag rc=$rc results=$(find $out -name result.json | wc -l) nova=$(grep -c '^\[NOVA\] kept' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  r=$(find $o -name result.json | wc -l); k=$(grep -c '^\[NOVA\] kept' $o.log); t=$(grep -c Traceback $o.log); e=$(grep -c 'EMPTY terminal answer' $o.log)
  grep '^\[NOVA\] kept' $o.log | cut -c1-400 >> $LOG
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$t" -eq 0 ] && [ "$e" -eq 0 ]; then echo "PASS results=$r nova=$k tb=$t empty=$e" > $K/SMOKE_GATE; else echo "FAIL results=$r nova=$k tb=$t empty=$e" > $K/SMOKE_GATE; fi

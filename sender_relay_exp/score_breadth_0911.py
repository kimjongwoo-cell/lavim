import json, glob, os, statistics as st, re, importlib.util, sys
TAG=sys.argv[1] if len(sys.argv)>1 else "evqa"
K="/home/users/whddn12316/wsi_latent_0915_decode_hj/sender_relay_exp/runs/vcut_breadth"
DS=json.load(open(f"/home/users/whddn12316/wsi_latent_0915_decode_hj/sender_relay_exp/smoke/{TAG}24.json"))
...
ref=load(f'{TAG}_id'); mref=margins(f'{TAG}_id')
print(f"{'arm':16s} {'n(common)':>9s} {'correct':>7s} {'id-correct':>10s} {'BACC':>6s} {'flip/id':>8s} {'M_V%':>6s} {'margin':>7s} {'id-margin':>9s}")
for a in [f'{TAG}_id',f'{TAG}_zeroR',f'{TAG}_zeroA',f'{TAG}_zeroA_L12_23']:
    t=load(a)
    if not t: continue
    ...
    d=[mref[i]-mg[i] for i in mc]; hurt=sum(x>0.5 for x in d); help_=sum(x<-0.5 for x in d)
    print(f"{a:16s} {len(common):9d} {ct:7d} {cr:10d} {bacc({i:t[i] for i in common}):6.3f} {fl:5d}/{len(common):<2d} {mv(a):6.2f} {mm:7.2f} {mr:9.2f}  dmA med {st.median(d) if d else float('nan'):+.2f} hurt/help {hurt}/{help_}")

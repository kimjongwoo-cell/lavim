...
log "CLAIM gpu$G shard=$SHARD arm=$ARM variant=$VARIANT nav4_md5=$(md5sum $REPO/vision_text_mas/onepass_navigation_nav4.py | cut -c1-8) navigator_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) eovc_md5=$(md5sum $REPO/memory/eovc.py | cut -c1-8)"
...
if [ "$SHARD" = smoke ]; then
  ...
  if [ "$r" -eq 2 ] && [ "$n4" -gt 0 ] && [ "$t" -eq 0 ] && [ "$em" -eq 0 ] \
    ...
    echo "PASS results=$r nav4=$n4 prunev4=$v4 eovc=$ev empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  else
    echo "FAIL results=$r nav4=$n4 prunev4=$v4 eovc=$ev need_eovc=$need_ev need_prunev4=$need_v4 empty=$em tb=$t" > $K/SMOKE_GATE_$ARM
  fi

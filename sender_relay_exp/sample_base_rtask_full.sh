#!/bin/bash
# 0916 FULL RUN: 0913-board Latent base (nav3, patch 12, step 10, NO C1/NTRS) + sampling decode
#   (--temperature 0.6 --top-p 0.95 --seed 42 --answerer-do-sample, same flags as sample_gtex_eff.sh)
...
K=$REPO/sender_relay_exp/runs/sample_base_rtask_nav3
...
run() {
  ...
  cd $REPO && env \
    ...
    --temperature 0.6 --top-p 0.95 --seed 42 --deterministic --answerer-do-sample --no-answerer-thinking \

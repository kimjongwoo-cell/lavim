...
if [ "$SHARD" = smoke ]; then
  ...
  pt=$(grep -c '^\[PlanTargets\]' $o.log); need_pt_zero=0; { [ "$ARM" = nonav2 ] || [ "$ARM" = latplan ] || [ "$ARM" = plipext ] || [ "$ARM" = plipnp ] || [ "$ARM" = navnp ] || [ "$ARM" = navsel ] || [ "$ARM" = plipsel ] || [ "$ARM" = nova_vn6g2_latplan ] || [ "$ARM" = nova_vn6g2_merge_latplan ]; } && need_pt_zero=1

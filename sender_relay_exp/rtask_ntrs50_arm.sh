#!/bin/bash
#   ARM=ntrs50       : NTRS 50% (VLMAS_C1_QASC=1 SELECT=ntrs KEEP=0.5)                -> runs/rtask_ntrs50_nav3
...
case $ARM in
  ntrs50) K=$REPO/sender_relay_exp/runs/rtask_ntrs50_nav3; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1" ;;
  *) echo bad arm; exit 2 ;;
esac
...
if [ "$SHARD" = smoke ]; then
  ...
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$t" -eq 0 ]; then echo "PASS results=$r kept1536=$k tb=$t" > $K/SMOKE_GATE; else echo "FAIL results=$r kept1536=$k tb=$t" > $K/SMOKE_GATE; fi

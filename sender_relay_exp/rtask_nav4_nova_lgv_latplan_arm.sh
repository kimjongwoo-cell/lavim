#!/bin/bash
# 09-19 COPY for NOVA Final (Early-LGV L2): hooks tmp_hj/nova_lgv_latplan_hooks (secld evidence -> nova_lgv.evidence_lgv, block-2 capture), ROOT runs/nav4_lgv_latplan, arm nova_lgv_latplan.
...
#   ROOT runs/nav4_lgv_latplan, own claim dir, hooks tmp_hj/nova_lgv_latplan_hooks (chains lpvh_ntrs_hooks, then patches secld_select.evidence).
...
ROOT=${OUTROOT:-$REPO/sender_relay_exp/runs/nav4_lgv_latplan}
VARIANT=base
case $ARM in
  ...
  nova_lgv_latplan) K=$ROOT/nova_lgv_latplan; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=nova VLMAS_NOVA_KEEP=0.25 VLMAS_C1_META_BRIDGE=1 VLMAS_NOVA_LGV=1 VLMAS_PLAN_LATENT_ONLY=1" ;;
...
if [ "$SHARD" = smoke ]; then
  ...
  need_nv=0; { [ "$ARM" = nova ] || [ "$ARM" = nova_lgv_latplan ]; } && need_nv=2
  need_n2=0; [ "$ARM" = nova_lgv_latplan ] && need_n2=2

#!/bin/bash
# Decode what the Reasoner would have said: after its latent steps, continue generating
# from the handoff cache until EOS. 5 sets x 20.
# usage: latdec_chain.sh <wait_pid|-> <gpu>
set -u
WAITPID=$1; GPU=$2
...
K=$TREE/sender_relay_exp/runs/latdec
...
run_one () {
  ...
  cd $TREE && env VLMAS_LATDEC=$K/latdec_$NAME.jsonl \
    VLMAS_LATDEC_STAGES=evidence_planner,navigator,reasoner VLMAS_LATDEC_CONT=512 "$@" \
  ...
  echo "DONE $NAME rc=$? rows=$(wc -l < $K/latdec_$NAME.jsonl 2>/dev/null || echo 0) marks=$(grep -c '^\[LatDec\] case' $K/$NAME.log) err=$(grep -c handoff_error $K/$NAME.log) tb=$(grep -ci traceback $K/$NAME.log) results=$(find $K/$NAME -name result.json 2>/dev/null | wc -l)" >> $K/chain_gpu$GPU.log
}
...
run_one smoke2 gtex2.json CUDA_LAUNCH_BLOCKING=1
n=$(grep -c '^\[LatDec\] case' $K/smoke2.log); tb=$(grep -ci traceback $K/smoke2.log)
res=$(find $K/smoke2 -name result.json 2>/dev/null | wc -l)

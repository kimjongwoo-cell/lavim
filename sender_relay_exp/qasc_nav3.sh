#!/bin/bash
# C1 #14 only: nav3 + Question-Agnostic Salience-Coverage Log-Det Selection with Cross-Scale Redundancy
#   (memory/qasc_select.py, VLMAS_C1_QASC=1; B = 25% of Reasoner visual tokens, vision block = last, no relay,
#   no C2). Base = nav3 base runs (runs/nav3_prompt1_base_*); matched budget to Pruning-B.
#   navigator  nav3 = VLMAS_NAV3_INDEPENDENT=1 + VLMAS_NAV2=1 + CROSS_SCALE_ROUTER=0 + prompt1, sdpa
#   base       4B Thinking · patch 12 · max-model-len 12288 · latent step 10 · structured_json
# Full sets (EVQA 128 · GTEx 190 · SB 197 · TCGA 221 · PANDA 196), sharded by index % 3 over GPU 6/7/8.
#
# usage: qasc_nav3.sh <gpu> <shard 0|1|2> <smoke|wait> [after-file]
#   smoke: 2-case gtex2 smoke + gate first (writes runs/qasc_nav3/smoke_gate.txt)
#   wait : wait for smoke_gate.txt == PASS
# Re-running resumes (indices with result.json under runs/qasc_nav3/<ds>/ are skipped).
set -u
G=${1:?gpu}; SHARD=${2:?shard}; ROLE=${3:?smoke|wait}; AFTER=${4:-}
...
K=$REPO/sender_relay_exp/runs/qasc_nav3
...
if [ -n "$AFTER" ]; then log "WAIT for $AFTER"; until [ -f "$AFTER" ]; do sleep 60; done; fi
log "WAIT gpu$G"
...
log "CLAIM gpu$G shard=$SHARD nav3_md5=$(md5sum $REPO/vision_text_mas/onepass_navigator.py | cut -c1-8) qasc_md5=$(md5sum $REPO/memory/qasc_select.py | cut -c1-8) backbone_md5=$(md5sum $REPO/backbone/qwen3vl.py | cut -c1-8) engine_md5=$(md5sum $REPO/vision_text_mas/latent_qwen_engine.py | cut -c1-8)"
...
run() {
  ...
  local pfr; pfr=$(grep -c '^\[QASC\] kept' $out.log)
  local res; res=$(find $out -name result.json | wc -l)
  log "DONE $tag rc=$rc results=$res canonical=$canon qasc=$pfr tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log)"
...
if [ "$ROLE" = smoke ]; then
  ...
  if [ "$res" -eq 2 ] && [ "$canon" -eq 0 ] && [ "$pfr" -ge 2 ] && [ "$tb" -eq 0 ]; then
    echo PASS > $K/smoke_gate.txt; log "SMOKE PASS results=$res canonical=$canon qasc=$pfr tb=$tb"
  else
    echo "FAIL results=$res canonical=$canon qasc=$pfr tb=$tb" > $K/smoke_gate.txt; log "SMOKE FAIL results=$res canonical=$canon qasc=$pfr tb=$tb"; exit 1
  fi
fi

#!/bin/bash
#   ARM=r2reasoner : Relay2를 Reasoner→Answerer 경계에만 (--variant latent_kv_relay2)
#   ARM=r2nav      : Relay2를 Navigator→Reasoner 경계에만 (--variant latent_kv_relay2_navigator)
#   ARM=relh : 디코딩 수정 + NTRS 50% + C2 13 RELH(VLMAS_RELH=1, 역할 종료 latent 열 1개의 key만 소비자 경계 좌표로 재회전) -> runs/rtask_relh_evqa
#   ARM=relh_identity : 같은 경로, p_handoff=p_endpoint (native 재현 확인)
#   ARM=relay2 : 디코딩 수정 + NTRS 50% + Relay2 dual(--variant latent_kv_relay2_dual: 시각에 붙은 Reasoner latent step만 다음 역할 캐시에 재노출) -> runs/rtask_relay2_evqa
...
case $ARM in
  ...
  r2reasoner) K=${OUTROOT:-$REPO/sender_relay_exp/runs/rtask_r2split_evqa}/reasoner; VARIANT=latent_kv_relay2; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1" ;;
  r2nav) K=${OUTROOT:-$REPO/sender_relay_exp/runs/rtask_r2split_evqa}/navigator; VARIANT=latent_kv_relay2_navigator; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1" ;;
  relh) K=$REPO/sender_relay_exp/runs/rtask_relh_evqa; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1 VLMAS_RELH=1" ;;
  relh_identity) K=$REPO/sender_relay_exp/runs/rtask_relh_evqa; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1 VLMAS_RELH=identity" ;;
  relay2) K=$REPO/sender_relay_exp/runs/rtask_relay2_evqa; VARIANT=latent_kv_relay2_dual; ARMENV="VLMAS_C1_QASC=1 VLMAS_C1_SELECT=ntrs VLMAS_QASC_KEEP=0.5 VLMAS_C1_META_BRIDGE=1" ;;
...
run() {
  ...
  cd $REPO && env \
    ...
    VLMAS_CROSS_SCALE_ROUTER=0 VLMAS_EFF_DUMP=1 VLMAS_ANSWERER_GRADE_RULE=1 VLMAS_REASONER_TASK="$RTASK" \
  ...
  local rc=$?
  log "DONE $ARM $tag rc=$rc results=$(find $out -name result.json | wc -l) psfp_case=$(grep -c '^\[PSFP\] case' $out.log) ladr_case=$(grep -c '^\[LADR\] case' $out.log) realloc=$(grep -c '^\[reallocate\]' $out.log) relay2=$(grep -c 'LatentKVRelay2' $out.log) relh_case=$(grep -c '^\[RELH\] case' $out.log) relh_skip=$(grep -c '^\[RELH\] SKIP' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) ntrs1536=$(grep -c '^\[NTRS\] kept 1536' $out.log) c1meta_ok=$(grep -c '^\[C1META\] keep .* used=True meta=ok' $out.log) empty=$(grep -c 'EMPTY terminal answer' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}
...
if [ "$SHARD" = smoke ]; then
  ...
  t=$(grep -c Traceback $o.log); c=$(grep -cE '^\[(LRPSR|PSFP|LADR|RELH)\] case' $o.log); rel2=$(grep -c 'LatentKVRelay2' $o.log); rl=$(grep -c '^\[reallocate\]' $o.log); sk=$(grep -cE '^\[(LRPSR|PSFP|LADR)\] SKIP' $o.log); em=$(grep -c 'EMPTY terminal answer' $o.log)
  md=$(grep -oE '(maxdiff_identity=[0-9.e+-]*|activated=[0-9]+ \([0-9.]+%\) argmax_flips=[0-9]+)' $o.log | tail -2 | tr '\n' ' ')
  GATE_CASE=2; case $ARM in realloc|realloc_v2|relay2|r2reasoner|r2nav) GATE_CASE=$c ;; esac
  if [ "$r" -eq 2 ] && [ "$k" -eq 2 ] && [ "$t" -eq 0 ] && [ "$c" -eq "$GATE_CASE" ] && [ "$sk" -eq 0 ] && [ "$em" -eq 0 ]; then
    echo "PASS results=$r kept1536=$k case=$c relay2=$rel2 realloc_lines=$rl skip=$sk empty=$em tb=$t $md" > $K/SMOKE_GATE_$ARM
  else
    echo "FAIL results=$r kept1536=$k case=$c relay2=$rel2 realloc_lines=$rl skip=$sk empty=$em tb=$t $md" > $K/SMOKE_GATE_$ARM
  fi

...
[ "$ARM" = navnp ] && HOOKS=$REPO/tmp_hj/ext_noplan_hooks   # VLMAS_EXT_PATCHES unset -> Navigator stays, only the Planner is skipped
...
[ "$ARM" = navnp ] && CLAIM=${CLAIMDIR:-$ROOT/claims_navnp}   # own GPU claims: never blocks or is blocked by plipext claims
...
if [ "$SHARD" = smoke ]; then
  ...
  pt=$(grep -c '^\[PlanTargets\]' $o.log); need_pt_zero=0; { [ "$ARM" = nonav2 ] || [ "$ARM" = latplan ] || [ "$ARM" = plipext ] || [ "$ARM" = plipnp ] || [ "$ARM" = navnp ]; } && need_pt_zero=1

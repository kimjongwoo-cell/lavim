#!/bin/bash
# Q-PRIS minimal verification plan (Notion section 7): three matched-budget arms.
#   qn_<ds>  VLMAS_QPRIS_Q=none      = PCRIS v2 (null-calibrated residual, no question term)
#   qt_<ds>  VLMAS_QPRIS_Q=true      = Q-PRIS
#   qs_<ds>  VLMAS_QPRIS_Q=shuffled  = Shuffled-Q control (previous case question)
# Same visual/provenance/budget/backend in all three; only the question term differs.
# usage: qpris_chain.sh <wait_pid|-> <gpu>
set -u
WAITPID=$1; GPU=$2
...
run_one smoke gtex2.json CUDA_LAUNCH_BLOCKING=1 VLMAS_QPRIS_Q=true VLMAS_QPRIS_DUMP=$K/smoke_dump
n=$(grep -c "\[WSIConsol\] qpris" $K/smoke.log); tb=$(grep -c Traceback $K/smoke.log)
res=$(find $K/smoke -name result.json 2>/dev/null | wc -l); qs=$(grep -c "q_src=true" $K/smoke.log)
echo "$(date +%H:%M) smoke: marks=$n q_src_true=$qs traceback=$tb result=$res" >> $K/chain_gpu$GPU.log
if [ "${n:-0}" -ge 2 ] && [ "${tb:-1}" -eq 0 ] && [ "${res:-0}" -ge 2 ] && [ "${qs:-0}" -ge 2 ]; then
  for ds in gtex tcga_expert_vqa tcga_slidebench panda tcga; do
    run_one qn_$ds dcs20_$ds.json VLMAS_QPRIS_Q=none
    run_one qt_$ds dcs20_$ds.json VLMAS_QPRIS_Q=true
    run_one qs_$ds dcs20_$ds.json VLMAS_QPRIS_Q=shuffled
  done

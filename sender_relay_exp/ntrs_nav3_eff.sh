...
# usage: ntrs_nav3_eff.sh <shard> <nshard> "<gpu list>" [handover_gpu handover_pid]
#   handover: wait for <handover_pid> (the previous launcher's running python on <handover_gpu>) to exit, then take over
#   that GPU's claim directly. VLMAS_EFF_DUMP=1 (per-case efficiency.json: FLOPs, TTFT, peak GPU memory).
...
LOG=$K/chain_shard$SHARD.log   # shared with ntrs_nav3.sh (resume)
mkdir -p $K $CLAIM
...
run() {
  ...
  local rc=$?
  log "DONE $tag rc=$rc results=$(find $out -name result.json | wc -l) ntrs=$(grep -c '^\[NTRS\] kept' $out.log) qasc=$(grep -c '^\[QASC\] kept' $out.log) tb=$(grep -c Traceback $out.log) oom=$(grep -ci 'out of memory' $out.log) eff=$(find $out -name efficiency.json | wc -l)"
}

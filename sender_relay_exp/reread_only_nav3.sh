#!/bin/bash
# Control run: nav3 · Context-Updated Answerer Query Visual Re-Read ONLY (no canonical addressing).
# Splits the nav3 + Canonical + ReRead drop into its re-read share (canonical-only control was stopped 09-15 03:35).
...
#   C2 read    matched context-updated query re-read at layer 27 with native visual mass kept
#                (VLMAS_ANSWERER_PFR=1 VLMAS_ANSWERER_PFR_LAYER=27); visual keys stay at native addresses
#                (no VLMAS_KV_ROUTE). Smoke gate requires canonical lines = 0 and PFR lines >= 2.
...
#
# usage: reread_only_nav3.sh <gpu> <shard 0|1|2> <smoke|wait>
#   smoke: 2-case gtex2 smoke + gate first (writes runs/canon_reread_nav3/smoke_gate.txt)
#   wait : wait for smoke_gate.txt == PASS
# Re-running resumes (indices with result.json under runs/reread_only_nav3/<ds>/ are skipped).
set -u
...
K=$REPO/sender_relay_exp/runs/reread_only_nav3

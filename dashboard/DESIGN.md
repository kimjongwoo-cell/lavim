...
- Structure: task name, state chip, queue order, progress fraction, success/failure counts, end-to-end wall seconds per question, completion ETA, queue-start wait, progress bar.
...
- Structure: three-stage cache-to-selection-to-Answerer flow, with a 10-by-32 logical-head map as the focal primitive.
- Semantics: cache categories remain distinct (text, visual, latent); the map indicates the K--K visual winner gate, not token deletion or a new KV tensor.
- Accessibility: stages and the selection criterion are named in text; the green sample cells are accompanied by a legend and formula.

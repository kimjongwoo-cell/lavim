...
ROOT: Final = Path("/home/users/whddn12316/wsi_latentmas_0806")
...
PAGE: Final = Path(__file__).resolve().with_name("index.html")
...
PERFORMANCE_WORKBOOK: Final = ROOT / "results/main_performance.xlsx"
...
_REFERENCE_PATH: Final = (
    ROOT / "runs/internvl_single_gpu7_735_20260819/eval/internvl_general_eval.jsonl"
)
REFERENCE_ROWS: Final = load_eval_rows(_REFERENCE_PATH) if _REFERENCE_PATH.exists() else {}
STRICT_METRICS: Final = load_strict_functions(
    Path(__file__).resolve().parent.parent / "eval/metrics.py"
)
    ...
    with _status_cache_lock:
        ...
        for backbone, model_size in groups:
            if backbone == "Qwen" and model_size in {"2B", "4B", "8B"}:
                modes = (
                    "Single", "Single No Thinking", "VLMAS", "Latent base", "Pruning V3",
                )
        ...
        for dataset in matrix_datasets:
            for model_size in ("2B", "4B", "8B"):
                for mode in matrix_modes:
                    steps = (None,) if mode in {"Single", "Single No Thinking", "VLMAS"} else (5, 10, 20, 30)
                    for latent_step in steps:
                        ...
                        if matches:
                            ...
                            ordered_quality.append(QualityMetric(
                                ...
                                model_size=model_size,
                                dataset=dataset,
                                thinking_mode="Thinking" if mode == "Single" else "No Thinking",
                            ))

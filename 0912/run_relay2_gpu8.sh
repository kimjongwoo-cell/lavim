...
indices=()
for index in {64..127}; do indices+=(--dataset-index "$index"); done

cd "$project_root"
CUDA_VISIBLE_DEVICES=8 PYTHONPATH="$project_root" PYTHONUNBUFFERED=1 \
  ...
  --output-root "$project_root/0912/runs/relay2_adaptive_gpu8" \

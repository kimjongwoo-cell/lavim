...
# VLMAS_RELAY_DEBUG=1 turns on conservation assertions inside the eager hook
# (sender-relay experiment); off by default so the hot path is unchanged.
import os as _os
_RELAY_DEBUG = _os.environ.get("VLMAS_RELAY_DEBUG", "") == "1"
...
class ReallocationProbe:
    ...
    # per-visual-column key norm captured once per context (sender-relay
    # diagnostics; never feeds the intervention itself)
    key_norm_vis: object = None
...
def reallocate(
    ...
    # Sender-relay experiment: pure static destination distribution (uniform /
    # sender_priority / shuffled_sender_priority). Only reachable when the
    # active relay_source opted in AND a weight vector is present; every
    # existing call keeps static_destination=False and is untouched. Degenerate
    # weights fall through to the canonical existing-attention destination.
    if static_destination and vis_w is not None:
        w_full = torch.zeros(k, dtype=torch.float32, device=aw.device)
        w_full[v] = vis_w[mask].float().clamp_min(0.0)
        w_sum = w_full.sum()
        if w_sum > 1e-9:
            af = af + moved * (w_full / w_sum)
            return af.to(aw.dtype)

    if not static_destination and vis_w is not None:
        w_full = torch.zeros(k, dtype=torch.float32, device=aw.device)
        w_full[v] = vis_w[mask].float().clamp_min(0.0)
        w_sum = w_full.sum()
        if w_sum > 1e-9:
...
def _patched(module, query, key, value, attention_mask, scaling, dropout=0.0, **kw):
    ...
    if CTX["on"] and li in CTX["layers"] and CTX["vis"] is not None and scope_ok:
        ...
        if probe is not None:
            ...
            # sender-relay diagnostics: capture per-visual-column key norm once
            # per context (read-only; does not touch aw)
            vis_ctx = CTX["vis"]
            if (
                probe.key_norm_vis is None
                and isinstance(vis_ctx, torch.Tensor)
                and vis_ctx.dtype != torch.bool
            ):
                kn = ks.float().norm(dim=-1).mean(dim=1)[0]
                vv = vis_ctx[vis_ctx < kn.shape[-1]]
                probe.key_norm_vis = kn[vv].detach().cpu()
        if CTX.get("mode") == "mask":
            ...
            static_destination = CTX.get("relay_source") in (
                "uniform",
                "sender_priority",
                "shuffled_sender_priority",
            )
            row_sums_before = aw.float().sum(-1) if _RELAY_DEBUG else None
            aw = reallocate(
                aw,
                CTX["vis"],
                CTX["alpha"],
            ...
            if row_sums_before is not None:
                drift = float((aw.float().sum(-1) - row_sums_before).abs().max())
                if static_destination:
                    # static destinations are exactly conserving by construction
                    assert drift < 5e-3, (
                        f"[RelayDebug] attention row-sum drift {drift:.2e} "
                        f"(layer {li}, relay_source={CTX.get('relay_source')})"
                    )
                elif drift >= 5e-3:
                    # PREEXISTING canonical behaviour: rows with ~zero visual
                    # attention have no recipients, so their moved mass (≤ α)
                    # vanishes. Kept as-is for backward compatibility; surface
                    # it in debug mode instead of failing the run.
                    print(
                        f"[RelayDebug] canonical row-sum leak {drift:.2e} "
                        f"(≤ alpha={CTX['alpha']:.3f}, layer {li}) — "
                        "preexisting canonical behaviour on visually-blind rows",
                        flush=True,
                    )
        if probe is not None:

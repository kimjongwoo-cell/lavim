...
def spectral_energy_keep_ratio(
    ...
    *,
    ...
    """Return E-AdaPrune's bounded budget using its q=2 randomized SVD."""
    if visual_features.ndim != 2 or visual_features.shape[0] < 1:
        raise ValueError("visual features must be a non-empty matrix")
    ...
    max_rank = min(
        features.shape[0],
        features.shape[1],
        math.ceil(features.shape[0] * max_keep_ratio) + 10,
    )
    ...
    projected = features @ omega
    for _ in range(2):
        projected = features @ (features.T @ projected)
        projected = torch.linalg.qr(projected, mode="reduced").Q
    compressed = projected.T @ features
    singular_values = torch.linalg.svdvals(compressed)
    energy = singular_values.square()
    total = features.square().sum()
    if not bool(torch.isfinite(total)) or float(total) <= 0.0:
        return min_keep_ratio
    cumulative = energy.cumsum(dim=0) / total
    rank = int(torch.searchsorted(cumulative, energy_ratio).item()) + 1
    raw_ratio = rank / visual_features.shape[0]
    return min(max_keep_ratio, max(min_keep_ratio, raw_ratio))
...
class PruneController:
    ...
    def __init__(self, args) -> None:
        ...
        self.parent_spatial_context = bool(
            getattr(args, "prune_parent_spatial_context", False)
        )
        self.parent_spatial_radius = int(
            getattr(args, "prune_parent_spatial_radius", 1)
        )
    ...
    def start(self, *, grids, vis_idx, sms: int, images, q2v, l_mid_layers,
        ...
        self.roi_grids = tuple(gh_gw)
        self.roi_boxes = tuple(
            tuple(int(value) for value in box) if box is not None else None
    ...
    def _hierarchy_fixed_ratio_keep(self) -> np.ndarray:
        ...
        if self.parent_spatial_context:
            from memory.relation_prune import relation_spatial_context_keep

            linked = relation_spatial_context_keep(
                keep=keep,
                score=score,
                roi_of_token=self.roi_of_token,
                parent_roi=self.parent_roi,
                roi_grids=self.roi_grids,
                roi_boxes=self.roi_boxes,
                radius=self.parent_spatial_radius,
                per_parent_cap=self.b_span,
            )
            required.update(np.flatnonzero(linked & ~keep).tolist())
        ...
        print(
            ...
            f"spatial-parent={self.parent_spatial_context})"
        )
        return keep

...
class SlideReuseCache:
    ...
    def _prepare(case: CaseInput) -> tuple[ManagedSlide, Image.Image, RootGrid]:
        from vision_text_mas.navigation_render import slide_thumbnail
...
def _run_case(
    *,
    ...
    engine._rpath_case_index = case.dataset_index   # Pathways probe: match passes by case
    engine._rpath_slide_id = case.item.slide_id     # VGain donor bank: wrong-slide control
    ...
    try:
        ...
        # Sender-relay experiment: persist the Answerer-handoff relay audit for
        # reallocation runs (engine leaves it None otherwise → no new file, so
        # base/pruning-only outputs stay byte-identical).
        relay_audit = getattr(engine, "last_relay_audit", None)
        if relay_audit is not None:
            (case_root / "relay_audit.json").write_text(
                json.dumps(relay_audit, indent=2), encoding="utf-8"
            )
            engine.last_relay_audit = None
        client.release()
        if temporary_navigation is not None:
            temporary_navigation.cleanup()
...
def run(
    ...
    patch_budget: Annotated[Literal[8, 12, 25], typer.Option("--patch-budget")] = 8,

...
class RootGrid:
    ...
    def eligible_ids(self) -> frozenset[int]:
        ...
        if eligible or not self.fractions:
            return eligible
        best_index = max(range(len(self.fractions)), key=self.fractions.__getitem__)
        return frozenset((best_index + 1,)) if self.fractions[best_index] > 0 else frozenset()
...
def prepare_root_grid(slide: SlideLike, *, thumbnail: Image.Image) -> RootGrid:
    ...
    if not eligible and fractions:
        best_index = max(range(len(fractions)), key=fractions.__getitem__)
        if fractions[best_index] > 0:
            eligible = frozenset((best_index + 1,))
    if not eligible:
        raise PipelineFailure(
            code=FailureCode.INSUFFICIENT_CANDIDATES,
            stage="navigator",
            detail="whole-slide thumbnail has no detectable stained tissue",
        )
    return RootGrid(
...
def _best_x5_child(
    ...
    *,
    ...
    try:
        # A root cell is >= EXACT_X5_SIDE on both sides unless the slide itself is narrower (PANDA: 11/196
    ...
    for rank, requested_root_id in enumerate(navigation.root_ids, start=1):
        try:
            ...
            if (
                allow_partial
                and patches
                and failure.code is FailureCode.INSUFFICIENT_CANDIDATES
            ):
                break
            raise

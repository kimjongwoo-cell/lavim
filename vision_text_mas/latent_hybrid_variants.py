...
class NativeHybridPruneConfig:
    ...
    query_adaptive_lambda: float = 0.5
    query_pathology_context: bool = False
    atp_sap: bool = False
    atp_all_layers: bool = False
    step_keep_ratios: tuple[float, ...] | None = None
    relative_saliency_floor: float | None = None
    cross_scale_consolidation: bool = False
    cross_scale_margin: float = 0.05
    morphology_coverage_rescue: bool = False
    parent_spatial_context: bool = False
    parent_spatial_radius: int = 1
    prune_hierarchy_all_roles: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.parent_spatial_radius < 0:
            raise ValueError("parent spatial context radius must be non-negative")
        ...
        if self.cross_scale_margin < 0.0:
            raise ValueError("cross-scale consolidation margin must be non-negative")
        ...
        if self.spectral_energy_ratio is not None and not 0.0 < self.spectral_energy_ratio <= 1.0:
            raise ValueError("spectral energy ratio must lie in (0, 1]")
        if not 0.0 < self.spectral_min_keep_ratio <= self.spectral_max_keep_ratio <= 1.0:
            raise ValueError("spectral keep bounds must satisfy 0 < min <= max <= 1")
        if not 0.0 <= self.query_adaptive_lambda <= 1.0:
            raise ValueError("query-adaptive lambda must lie in [0, 1]")
...
class _BackbonePruneArgs:
    ...
    prune_roles: tuple[str, ...] | None = None
    ...
    prune_spectral_energy_ratio: float | None = None
    prune_spectral_min_keep_ratio: float = 0.25
    prune_spectral_max_keep_ratio: float = 0.50
    prune_query_adaptive: bool = False
    prune_query_adaptive_lambda: float = 0.5
    prune_query_pathology_context: bool = False
    prune_atp_sap: bool = False
    prune_atp_all_layers: bool = False
    prune_step_keep_ratios: tuple[float, ...] | None = None
    prune_relative_saliency_floor: float | None = None
    prune_cross_scale_consolidation: bool = False
    prune_cross_scale_margin: float = 0.05
    prune_morphology_coverage_rescue: bool = False
    prune_parent_spatial_context: bool = False
    prune_parent_spatial_radius: int = 1
    prune_hierarchy_all_roles: bool = False
    ...
    *,
    ...
    if prune_config is not None:
        backbone._prune_args = _BackbonePruneArgs(
            ...
            prune_roles=prune_config.roles,
            ...
            prune_spectral_energy_ratio=prune_config.spectral_energy_ratio,
            prune_spectral_min_keep_ratio=prune_config.spectral_min_keep_ratio,
            prune_spectral_max_keep_ratio=prune_config.spectral_max_keep_ratio,
            prune_query_adaptive=prune_config.query_adaptive,
            prune_query_adaptive_lambda=prune_config.query_adaptive_lambda,
            prune_query_pathology_context=prune_config.query_pathology_context,
            prune_atp_sap=prune_config.atp_sap,
            prune_atp_all_layers=prune_config.atp_all_layers,
            prune_step_keep_ratios=prune_config.step_keep_ratios,
            prune_relative_saliency_floor=prune_config.relative_saliency_floor,
            prune_cross_scale_consolidation=prune_config.cross_scale_consolidation,
            prune_cross_scale_margin=prune_config.cross_scale_margin,
            prune_morphology_coverage_rescue=prune_config.morphology_coverage_rescue,
            prune_parent_spatial_context=prune_config.parent_spatial_context,
            prune_parent_spatial_radius=prune_config.parent_spatial_radius,
            prune_hierarchy_all_roles=prune_config.prune_hierarchy_all_roles,
        )
...
def latent_reallocation(
    ...
    if alpha == 0.0 and not config.adaptive and not config.collect_visual_grounding:
        yield None
        return
    model = cast(Any, getattr(backbone, "model", None))
    language_model = cast(Any, getattr(backbone, "lm", None))
    previous_model_attention: str | None = None
    previous_language_attention: str | None = None
    if config.collect_visual_grounding:
        # Each latent call has Q=1, so eager attention can score Q_latent to
        # K_visual without materializing a full 12k-by-12k attention output.
        previous_model_attention = model.config._attn_implementation
        previous_language_attention = language_model.config._attn_implementation
    ...
    with reallocating(
        ...
        visual_patch_ids=(
            visual_patch_ids if config.hierarchy_context_reasoner else None
        ),
        parent_patch_indices=(
            parent_patch_indices if config.hierarchy_context_reasoner else ()
        ),
        parent_anchor_strength=(
            config.parent_anchor_strength if config.hierarchy_context_reasoner else 0.0
        ),
    ) as probe:
        try:
            yield probe
        finally:
            if previous_model_attention is not None:
                model.config._attn_implementation = previous_model_attention
            if previous_language_attention is not None:
                language_model.config._attn_implementation = previous_language_attention

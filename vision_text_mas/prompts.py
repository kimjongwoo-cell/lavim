...
def answerer_prompt(
    *,
    ...
    if choices:
        ...
        # 0916 VLMAS_ANSWERER_GRADE_RULE=1 (PANDA only, off = unchanged): the choices are the
        # ISUP grade values 0-5, and the numbering of the CHOICE lines is offset from them by
        # one. State the value range instead of the line index so the answer is a grade.
        if os.environ.get("VLMAS_ANSWERER_GRADE_RULE", "").strip() == "1" and \
                [c.strip() for c in choices] == [str(i) for i in range(len(choices))]:
            answer_rule = (
                "The answer must be exactly one of the grade values "
                + ", ".join(str(i) for i in range(len(choices)))
                + " (the value itself, not the CHOICE line number)."
            )
    elif open_options:

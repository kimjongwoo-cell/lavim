...
class EvidenceRound(FrozenModel):
    """One through twenty-five patches retrieved by one navigation action."""

    round_number: int = Field(ge=1, le=3)
    action: NavigationAction
    patches: tuple[PatchMetadata, ...] = Field(min_length=1, max_length=25)
    ...
    def require_consistent_patch_identity(self) -> Self:
        ...
        if actual_ids != expected_ids:
            msg = "patch IDs must be ordered R{round}-P1 through the final rank"
...
class ReasonerReport(FrozenModel):
    """One-through-twenty-five-patch observations and a non-diagnostic synthesis."""

    patches: tuple[PatchObservation, ...] = Field(min_length=1, max_length=25)
...
class RunResult(FrozenModel):
    ...
    patches: tuple[PatchMetadata, ...] = Field(min_length=1, max_length=75)
    ...
    def require_valid_patch_budget_per_round(self) -> Self:
        ...
        for round_number in expected_rounds:
            ...
            if not 1 <= patch_count <= 25:
                msg = "each evidence round must contain one through twenty-five patches"
                raise ValueError(msg)

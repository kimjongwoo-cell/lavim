#!/bin/bash
# LCVR (Notion C2 #28) B arm, 5 diagnostic cases: grounded replay + consensus re-read at the terminal Answerer row.
...
ARGS=(--variant base --backbone qwen3-vl --model /home/users/whddn12316/models/Qwen3-VL-4B-Thinking --slide-root ${SLIDE_ROOT:-$DATA/slides} --device cuda:0
...
for mode in $MODES; do
  ...
  extra="$NOVA VLMAS_RNLCR=ground VLMAS_GLVR=$OUT/$mode/glvr.jsonl VLMAS_GLVR_CHAIN=nova_rho_latplan_hooks VLMAS_LCVR=$OUT/$mode/lcvr.jsonl VLMAS_LCVR_LAMBDA=${LAMBDA:-0.75}"
  ( cd $REPO && env CUDA_VISIBLE_DEVICES=$g PYTHONPATH=$REPO/tmp_hj/lcvr:$REPO PYTHONUNBUFFERED=1 $BASE_ENV $extra VLMAS_REASONER_TASK="$RTASK" \
...
wait; echo "ALL DONE $MODES $(date +%H:%M)" | tee -a $OUT/launch.log

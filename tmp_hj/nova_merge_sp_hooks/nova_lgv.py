...
# ---------------------------------------------------------------------------------------------------------------
# 09-19 candidate E: SPATIALLY CONSTRAINED merge. Same size-weighted, norm-preserving fold as merge_dropped, but a
# dropped token may only go to a kept token within Chebyshev radius R on the crop token grid (VLMAS_NOVA_MERGE_R,
# default 2); among those the most similar (cos of normalised z) wins, and a dropped token with no kept token inside
# R is discarded as in plain NOVA. Token count / positions / KV unchanged (25%).
@torch.no_grad()
def merge_dropped_sp(feats: torch.Tensor, keep: torch.Tensor, grids) -> torch.Tensor:
    R = float(_os.environ.get("VLMAS_NOVA_MERGE_R", "2"))
    f = feats.float(); z = F.normalize(f, dim=-1)
    kept_idx = torch.nonzero(keep, as_tuple=True)[0]
    out = f[kept_idx].clone()
    ...
    off, merged, far, sims, dists = 0, 0, 0, [], []
    for gh, gw in grids:
        n = gh * gw
        sl = torch.arange(off, off + n, device=f.device); off += n
        rows = torch.arange(gh, device=f.device).repeat_interleave(gw).float()
        cols = torch.arange(gw, device=f.device).repeat(gh).float()
        km = keep[sl]
        k = sl[km]; p = sl[~km]
        if k.numel() == 0 or p.numel() == 0:
            continue
        d = torch.maximum((rows[~km][:, None] - rows[km][None, :]).abs(),
                          (cols[~km][:, None] - cols[km][None, :]).abs())   # Chebyshev on the token grid
        ok = d <= R
        sim = z[p] @ z[k].T
        best = sim.masked_fill(~ok, -2.0).argmax(1)
        has = ok.any(1)
        far += int((~has).sum())
        if int(has.sum()) == 0:
            continue
        bp = p[has]; bk = k[best[has]]
        sims.append(sim[has, best[has]]); dists.append(d[has, best[has]])
        tgt = pos_in_out[bk]
        acc.index_add_(0, tgt, f[bp]); cnt.index_add_(0, tgt, torch.ones_like(tgt, dtype=cnt.dtype))
        merged += int(bp.numel())
    mean = acc / cnt[:, None]
    res = F.normalize(mean, dim=-1) * out.norm(dim=-1, keepdim=True)
    sim = torch.cat(sims) if sims else torch.zeros(1, device=f.device)
    dst = torch.cat(dists) if dists else torch.zeros(1, device=f.device)
    print(f"[NOVAMERGESP] R={R:g} kept={int(kept_idx.numel())} merged={merged} unmerged_far={far} "
          f"per_kept_mean={float(cnt.mean()) - 1:.2f} per_kept_max={int(cnt.max()) - 1} "
          f"assign_cos_mean={float(sim.mean()):.3f} assign_dist_mean={float(dst.mean()):.2f} "

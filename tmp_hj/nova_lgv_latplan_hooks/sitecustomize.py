...
if os.environ.get("VLMAS_NOVA_LGV", "") == "1":
    ...
    def _patch_nova(module):
        ...
        def select_vision_features(bb, pixel_values, grid_thw):
            ...
            counts = [int(t) * int(h) * int(w) // unit for t, h, w in _t.as_tensor(grid_thw).tolist()]
            return nova_lgv.merge_dropped(full, keep, counts), keep
        module.select_vision_features = select_vision_features
        print(f"[NOVALGV] wrapped memory.nova_select.select_vision_features (block-2 capture, merge={do_merge})", flush=True)

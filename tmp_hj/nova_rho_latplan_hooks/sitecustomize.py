"""NOVA rho + latplan hook. Chains tmp_hj/latplan_hooks (Planner latent-only hand-off; itself chains lpvh_ntrs_hooks),
then with VLMAS_NOVA_RHO=1 patches memory.secld_select.evidence -> memory.nova_rho_select.evidence_rho."""
import os
import sys

_HERE = os.path.dirname(os.path.abspath(__file__))
_BASE_HOOKS = os.path.join(os.path.dirname(_HERE), "lpvh_ntrs_hooks")  # c1_meta_bridge lives here; latplan_hooks does not add it

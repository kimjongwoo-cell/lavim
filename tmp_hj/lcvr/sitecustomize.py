...
if os.environ.get("VLMAS_GLVR", "").strip():
    ...
    def _patch_rnlcr_terminal(module):
        ...
        def terminal(engine, cache, position_cursor, **kwargs):
            ...
            if os.environ.get("VLMAS_LCVR", "").strip():
                LC = importlib.import_module("lcvr")
                inner = LC.terminal(engine, cache, system_prompt=kwargs.get("system_prompt"),
                                    user_prompt=kwargs.get("user_prompt"), json_prefix=kwargs.get("json_prefix"),
                                    case_index=int(kwargs.get("case_index", -1)))
                with outer as stats, inner:
                    yield stats
                return
            if os.environ.get("VLMAS_RCVR", "").strip():
                RC = importlib.import_module("rcvr")
                inner = RC.terminal(engine, cache, system_prompt=kwargs.get("system_prompt"),
                                    user_prompt=kwargs.get("user_prompt"), json_prefix=kwargs.get("json_prefix"),
                                    case_index=int(kwargs.get("case_index", -1)))
                with outer as stats, inner:
                    yield stats
    ...
    def _patch_rnlcr_all(module):
        ...
        try:
            if (float(os.environ.get("VLMAS_GLVR_APPLY", "0") or 0) or os.environ.get("VLMAS_RCVR", "").strip()
                    or os.environ.get("VLMAS_LCVR", "").strip()):
    ...
    def _patch_engine_coverage(module):
        ...
        def append(self, *args, **kwargs):
            b = CV.beta()
            gencap = os.environ.get("VLMAS_GLVR_GENCAP", "") == "1"
            if kwargs.get("stage") != "reasoner" or (b == 0.0 and not gencap):
    ...
    try:
        if float(os.environ.get("VLMAS_GLVR_COV", "0") or 0) or os.environ.get("VLMAS_GLVR_GENCAP", "") == "1":

...
class FastTerminalRelay:
    ...
    def _prep(self, li: int, keys, vals, device):
        ...
        if st is None:
            ...
            st = {"Kp": K[:, :self.base, :].float(),
                  "Vd": V.index_select(1, don).float(),                 # [Hkv, Nd, D]
                  "don": don, "lat": self.latent.to(device), "U": U}
            self._layer[li] = st
        return st
        ...
        def post(module, args, kwargs, output):
            ...
            try:
                with torch.no_grad():
                    ...
                    s_pre = torch.matmul(qg, st["Kp"].transpose(-2, -1))                        # [Hkv, G*R, base]
                    s_new = torch.matmul(qg, K[:, self.base:, :].float().transpose(-2, -1))     # [Hkv, G*R, L-base]
                    scores = torch.cat([s_pre, s_new], dim=-1).view(H, R, L) * module.scaling
                    past = L - T
                    col = torch.arange(L, device=hidden.device)[None, None, :]
                    row = (past + rows)[None, :, None]
                    A = torch.softmax(scores.masked_fill(col > row, float("-inf")), dim=-1)     # [H, R, L]
                    Ad = A.index_select(-1, st["don"])                                          # [H, R, Nd]
                    rho_B = Ad.sum(-1, keepdim=True)                                            # [H, R, 1]
                    m_B = torch.einsum("kgrn,knd->kgrd", Ad.view(Hkv, G, R, -1), st["Vd"]).reshape(H, R, Hd)
                    al = A.index_select(-1, st["lat"])
                    ...
                    delta = self.lam * (rho_B * u - m_B)                                        # [H, R, D]
                    out = output[0] if isinstance(output, tuple) else output
...
def fast_terminal_relay(bb, groups: dict, U_V: list, lam: float, base_len: int, n_prompt: int = 0):
    tr = FastTerminalRelay(bb, groups, U_V, lam, base_len, n_prompt=n_prompt)
    handles = [layer.self_attn.register_forward_hook(tr.attn_post(li), with_kwargs=True)
               for li, layer in enumerate(bb.lm.layers)]
    try:
        yield tr

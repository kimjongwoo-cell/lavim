...
for ds in RUNS:
    for cdir in cases(ds):
        ...
        with torch.no_grad():
            ...
            l2 = lgv(e2, C)
            W = {"LogDet-only": torch.ones(N, device=dev), "NOVA-ρ (대시보드)": rho_e,
        ...
        for k in tk:
            ...
            rec["arms"][k] = {"min": min(per), "sd": float(np.std(per)), "J_rho": J(k, "NOVA-ρ (대시보드)"), "J_logdet": J(k, "LogDet-only"),
...
json.dump(out, open(os.path.join(os.path.dirname(__file__), "variants_sel.json"), "w"), indent=1)

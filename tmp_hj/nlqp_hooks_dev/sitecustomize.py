...
No extra token, no cache column, no gain / temperature / top-k; one row of one generation step.
  ...
  VLMAS_NLQP_GAIN=<g>  rescale r per layer so |r| = g * |x_b| (0 / unset = natural scale). The natural scale is
                       ~1e-5 for rfnm and 0.6-0.9 for the latent-only softmax of NLQP; this sweeps between them.
  VLMAS_NLQP_CEIL=<jsonl>  ceiling probe (measurement only, answers unchanged): at the row that picks the first
                       answer-content token, re-run that row once per (direction, scale) and record the top tokens.
                       Directions = the m centered latent messages and the native-alpha combination; scales from
                       VLMAS_NLQP_CEIL_SCALES. Offline the gold answer's first token is looked up in each result,
                       so the probe answers "could ANY latent-derived query priming start the gold answer here?".
...
if _MODE:
    ...
    _GAIN = float(os.environ.get("VLMAS_NLQP_GAIN", "0") or 0)      # >0: rescale r to |r| = GAIN * |x_b| per layer
    _CEIL = os.environ.get("VLMAS_NLQP_CEIL", "").strip()           # ceiling probe output (measurement only)
    _CEIL_SCALES = tuple(float(x) for x in
                         os.environ.get("VLMAS_NLQP_CEIL_SCALES", "0.01,0.03,0.1,0.3").split(","))
        ...
        def _attn_pre(self, li, layer):
            def pre(module, args, kwargs):
                ...
                with torch.no_grad():
                    ...
                    x_b = self.x[li]
                    if _GAIN > 0 and float(r.float().norm()) > 0:
                        r = r * (_GAIN * float(x_b.float().norm()) / float(r.float().norm()))
    ...
    class _CeilProbe:
        """Measurement only. On a COPY of the pre-Answerer cache, re-run the row that picks the first answer-content
        token once per (latent direction, scale) and record the top tokens. Answers of the real run are untouched."""

        def __init__(self, bb, dirs):
            import torch
            self.bb, self.dirs, self.r = bb, dirs, None     # dirs: per layer [m, hidden] unit vectors
            self.handles, self.cap = [], {}
            for li, layer in enumerate(bb.lm.layers):
                self.handles.append(layer.register_forward_pre_hook(self._pre(li), with_kwargs=True))
                self.handles.append(layer.self_attn.q_proj.register_forward_hook(self._q(li)))
            self.handles.append(bb.lm.norm.register_forward_hook(self._norm))
        ...
        def _pre(self, li):
            def pre(module, args, kwargs):
                hs = kwargs.get("hidden_states", args[0] if args else None)
                self.hhat = getattr(self, "hhat", {})
                if self.r is None or hs is None or hs.dim() != 3 or hs.shape[1] < 2:
                    return None
                x_b = hs[0, -1].detach()
                rv = self.r[li] * float(x_b.float().norm())
                self.hhat[li] = module.input_layernorm((x_b + rv.to(x_b.dtype))[None, None])[0, 0]
                return None
            return pre

        def _q(self, li):
            def post(module, args, output):
                hh = getattr(self, "hhat", {}).pop(li, None)
                if hh is None:
                    return output
                out = output.clone()
                out[0, -1] = module(hh[None, None].to(output.dtype))[0, 0]
                return out
            return post
        ...
        def _norm(self, module, args, output):
            ...
            if h.shape[1] >= 2:
                self.cap["h"] = h[0, -1].detach()
            return output

        def run_one(self, r_per_layer, **kw):
            """r_per_layer: None (baseline) or per-layer unit vectors scaled later by |x_b|."""
            import torch
            from copy import deepcopy
            from vision_text_mas.latent_terminal import generate_terminal_json
            self.r, self.cap = r_per_layer, {}
            c = deepcopy(kw["cache"])
            try:
                generate_terminal_json(backbone=self.bb, cache=c, position_cursor=kw["cursor"],
                                       system_prompt=kw["system"], user_prompt=kw["user"],
                                       json_prefix=(kw["prefix"] or "") + ' "', max_new_tokens=1,
                                       temperature=0.0, top_p=1.0, do_sample=False)
            finally:
                del c
                self.r = None
            head = self.bb.model.lm_head
            with torch.no_grad():
                lp = torch.log_softmax(head(self.cap["h"].to(head.weight.dtype)[None])[0].float(), -1)
                top = torch.topk(lp, 50)
            return [int(i) for i in top.indices], [round(float(x), 4) for x in top.values]

    def _ceiling(client, bb, case, **kw):
        """One probe pass per (direction, scale) plus the unprimed baseline; written to VLMAS_NLQP_CEIL."""
        import torch
        ...
        t0 = time.time()
        m = mt[0].shape[0]
        dirs = []                                   # [k][layer] unit hidden vector
        with torch.no_grad():
            for k in range(m):
                per = []
                for li, X in enumerate(mt):
                    o = bb.lm.layers[li].self_attn.o_proj
                    v = o(X[k].reshape(1, 1, -1).to(o.weight.dtype))[0, 0].float()
                    per.append(v / v.norm().clamp_min(1e-9))
                dirs.append(per)
        p = _CeilProbe(bb, dirs)
        rec = {"case": case, "m": m, "scales": list(_CEIL_SCALES)}
        try:
            ids, lps = p.run_one(None, **kw)
            rec["base"] = [ids[:20], lps[:20]]
            rec["probe"] = []
            for k in range(m):
                for s in _CEIL_SCALES:
                    ids, lps = p.run_one([d * s for d in dirs[k]], **kw)
                    rec["probe"].append({"k": k, "s": s, "ids": ids[:20], "lp": lps[:20]})
        finally:
            p.remove()
        rec["sec"] = round(time.time() - t0, 2)
        with open(_CEIL, "a") as fh:
            fh.write(_json.dumps(rec) + "\n")
        print(f"[CEIL] case {case} dirs={m} scales={list(_CEIL_SCALES)} sec={rec['sec']} "
              f"base_top={rec['base'][0][0]}", flush=True)
    ...
    def _wrap_client(cls):
        ...
        def generate_final_text(self, **kw):
            ...
            if _CEIL and _S.get("ceil_done") != case:
                _S["ceil_done"] = case
                try:
                    _ceiling(self, bb, case, cache=self._state.kv, cursor=self._state.position_cursor,
                             system=kw.get("system_prompt"), user=kw.get("user_prompt"),
                             prefix=kw.get("terminal_prefix"))
                except Exception as exc:  # noqa: BLE001 - measurement only
                    import traceback
                    traceback.print_exc()
                    print(f"[CEIL] case {case} FAILED {exc!r}", flush=True)
            t0 = time.time()

#!/usr/bin/env python3
"""Exact scoring + efficiency for runs/sample_base_rtask_nav3 (copy tree) vs nav3 greedy base (same dataset_index).
...
C = Path("/home/users/whddn12316/wsi_latent_0915_decode_hj/sender_relay_exp/runs/sample_base_rtask_nav3")
...
for ds in targets:
    ...
    base_roots = sorted(R.glob(f"nav3_prompt1_base_{ds}_gpu*")) if ds != "panda" else [R / "nav3clamp_base" / "panda"]
    base, bsec = g(base_roots, ds)
    samp, _ = g([R / "sample_t06_nav3" / "base" / ds], ds)
    ...
    if sids:
        row["samp_noprompt"] = {"n": len(sids), "arm": sc(arm, sids), "samp": sc(samp, sids),

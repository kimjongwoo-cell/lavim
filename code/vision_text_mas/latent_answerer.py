...
LATENT_JUDGER_SYSTEM = (
    ...
    # 0717-faithful ANSWER_RULES (prompts/diagnosis.py::ANSWER_RULES). The latent
    # onepass path had dropped these, so OPEN answers drifted (her2 -> a definition,
    # vital_status -> "inconclusive", survival -> prose). These field-type rules
    # discipline the free-text answer the way 0717 / single_v7 already do.
    "Rules:\n"
...
def latent_answerer_prompt(
    *,
    ...
    """Expose the task while leaving all upstream evidence in latent state.

    Default is answer-only (`<answer>…</answer>`). Set include_rationale=True to
    also request a `<rationale>…</rationale>` tag.
    """
    choice_lines = "\n".join(
    ...
    # Latent path asks the Judger for the answer wrapped in an <answer> tag (plus an
    # optional <rationale> tag when include_rationale is set). decision_basis /
    # evidence_patches are filled server-side from the verifier decision and
    # confidence was always a copied constant, so they never travel. Tags (not JSON)
    # survive quotes / newlines / braces inside the rationale. The terminal decode
    # teacher-forces the leading "<answer>" prefix so a greedy Thinking model commits
    # to the tag envelope instead of rambling.
    if protocol is AnswererProtocol.BOXED:
        ...
        # 0717 / vLLM-style readout: a single flat JSON object. The terminal decode
        # teacher-forces the leading '{"answer":' so a greedy Thinking model commits
        # to the object instead of prose; keep it one line so a stray '}' inside the
        # value does not truncate the object mid-field.
...
class LatentAnswererAgent:
    ...
    def answer(
        ...
        *,
        ...
        _ = (thumbnail, memory, evidence_board, reasoner_reports)
        ...
        record = self._client.generate_final_text(
            system_prompt=LATENT_JUDGER_SYSTEM,

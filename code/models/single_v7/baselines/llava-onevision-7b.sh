...
#######################################
# LLaVA-OneVision-Qwen2-7B (HF-native) single-model WSI-VQA inference + eval.
#
...
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 bash baselines/llava-onevision-7b.sh
...
#   IMAGE_ROOT=/media/super/4TB/hj/SlideBench_BCNB/WSIs/WSIs bash baselines/llava-onevision-7b.sh
#######################################
...
LLAVAOV_ADAPTER="${LLAVAOV_ADAPTER:-$LATENTMAS_ROOT/baselines/inference/llava-onevision-7b-inference.py}"

TEST_JSON="${TEST_JSON:-/home/super/hj/HJ/WSI-VQA/dataset/WSI_captions/WsiVQA_test.json}"
INPUT_JSON="${INPUT_JSON:-$TEST_JSON}"
CHECKPOINT="${CHECKPOINT:-/media/super/4TB/hj/llava-onevision-qwen2-7b-ov-hf}"
LLM_URL="${LLM_URL:-}"
LLM_MODEL="${LLM_MODEL:-}"
...
DEFAULT_WORK_DIR="$RESULTS_ROOT/llava-onevision-7b/${RUN_ID}"
WORK_DIR="${WORK_DIR:-$DEFAULT_WORK_DIR}"

THUMBNAIL_DIR="${THUMBNAIL_DIR:-$SHARED_THUMBNAIL_DIR}"
ANSWERS_FILE="${ANSWERS_FILE:-$WORK_DIR/predictions/llavaov_predictions.jsonl}"
VIS_DIR="${VIS_DIR:-$WORK_DIR/vis}"
EVAL_DIR="${EVAL_DIR:-$WORK_DIR/eval}"
EVAL_JSON="${EVAL_JSON:-$EVAL_DIR/llavaov_summary.json}"
EVAL_CSV="${EVAL_CSV:-$EVAL_DIR/llavaov_details.csv}"
GENERAL_OUTPUT_JSONL="${GENERAL_OUTPUT_JSONL:-$EVAL_DIR/llavaov_general_eval.jsonl}"
INFERENCE_TIME_JSON="${INFERENCE_TIME_JSON:-$EVAL_DIR/inference_time.json}"
GENERAL_EVAL_DIR="${GENERAL_EVAL_DIR:-$EVAL_DIR/general}"
GENERAL_EVAL_NAME="${GENERAL_EVAL_NAME:-llava_onevision_7b_${DATASET_TAG}}"
GENERAL_EVAL_TXT="${GENERAL_EVAL_TXT:-$EVAL_DIR/general_eval.txt}"
LOG_DIR="${LOG_DIR:-$WORK_DIR/logs}"
RUN_LOG="${RUN_LOG:-$LOG_DIR/llava_onevision_7b.log}"
...
check_inputs() {
  ...
  require_file "$LLAVAOV_ADAPTER" "LLaVA-OneVision inference adapter"
  require_file "$GENERAL_EVALUATOR" "general WSI-VQA evaluator"
  if [[ -z "$LLM_URL" ]]; then
    require_dir "$CHECKPOINT" "LLaVA-OneVision checkpoint directory"
    require_file "$CHECKPOINT/config.json" "LLaVA-OneVision config"
  fi
...
main() {
  setup_logging
  log "LLaVA-OneVision-7B single-model WSI-VQA baseline"
  log "Checkpoint: $CHECKPOINT"
  log "Adapter: $LLAVAOV_ADAPTER"
